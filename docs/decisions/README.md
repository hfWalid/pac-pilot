...
Six questions raised in review remain **undecided**. They are not ADRs, because there is no decision
to record yet, and writing them up would falsely imply one. They are tracked on the Jira epics they
affect (and listed on the `Documentation` ticket):
...
3. `Intervention` persistence is not in M4's "migrations for all aggregates" — M7.5 reopens the
   server backbone.
4. Identity lands at M10, but `validatedBy` (the legal shield) and M8's idempotent ingestion both
   need a principal well before that.
5. The RGPD review sits at M11, but retention and minimisation shape the schema at M4 — "by design"
...
When one is settled, it gets an ADR.
