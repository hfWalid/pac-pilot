...
Read in this order. The Markdown docs are the source of truth; there is no Confluence.
...
Docs land in `docs/` at **M0-09**, together with `docs/decisions/` (one ADR per locked decision).
...
Requires JDK 21. Gradle comes from the wrapper — no local install needed.
